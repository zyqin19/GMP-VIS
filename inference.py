...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model_path', type=str, default='/home/qinzheyun/PycharmProjects/data/'
                                                          'pretrained/vistr/vistr_r50.pth',
    # parser.add_argument('--model_path', type=str, default='/home/qinzheyun/PycharmProjects/data/'
    #                                                       'output/vistr/vistr_r50/checkpoint0005.pth',
    ...
    # dataset parameters
    parser.add_argument('--img_path', default='/home/qinzheyun/Data/YouTube/vis/2021/valid/JPEGImages/')
    parser.add_argument('--ann_path', default='/home/qinzheyun/Data/YouTube/vis/2021/valid/instances.json')
    parser.add_argument('--save_path', default='/home/qinzheyun/PycharmProjects/data/'
                                               'output/vistr/vistr_r50/json/ytvis21')
...
def main(args):
    ...
    with torch.no_grad():
        ...
        for i in range(vis_num):
            ...
            if clip_num > 1:
                step = int((args.num_frames * clip_num - length) // (clip_num - 1))
            else:
                step = 0

            logits, boxes, masks = torch.Tensor([args.num_queries]), \
                                   torch.Tensor([args.num_queries]), \
                                   torch.Tensor([args.num_queries])

            if length >= num_frames:
                for i in range(int(clip_num)):
                    sub_file_names = file_names[i * ((length - args.num_frames) // clip_num):
                                                i * ((length - args.num_frames) // clip_num) + num_frames]

                    img_set = []
                    for k in range(num_frames):
                        im = Image.open(os.path.join(folder, sub_file_names[k]))
                    ...
                    sub_logits, sub_boxes, sub_masks = outputs['pred_logits'].softmax(-1)[0, :, :-1], \
                    ...
                    if i == 0:
                        ...
                        logits = torch.cat((logits, sub_logits[step * 10:, :]), 0)
                        boxes = torch.cat((boxes, sub_boxes[step * 10:, :]), 0)
                        masks = torch.cat((masks, sub_masks[step * 10:, :, :]), 0)

                pred_masks = F.interpolate(masks.reshape(length, num_ins, masks.shape[-2], masks.shape[-1]),
                                           (im.size[1], im.size[0]),
                                           mode="bilinear").sigmoid().cpu().detach().numpy() > 0.5
                pred_logits = logits.reshape(length, num_ins, logits.shape[-1]).cpu().detach().numpy()
            else:
                img_set = []
        ...
        with open(os.path.join(args.save_path, 'results.json'), 'w', encoding='utf-8') as f:
            json.dump(result, f)
        with zipfile.ZipFile(os.path.join(args.save_path, 'results.zip'), 'w') as z:
            z.write(os.path.join(args.save_path, 'results.json'), arcname='results.json')
