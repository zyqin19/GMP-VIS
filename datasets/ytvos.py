...
import torchvision
from util.pycocotools.ytvos import YTVOS
from util.pycocotools.ytvoseval import YTVOSeval
...
class YTVOSDataset:
    ...
    def __getitem__(self, idx):
        ...
        # n_x1, n_y1, n_x2, n_y2 = target['boxes'][0]
        # plt.imshow(img[0], interpolation="bicubic")
        # plt.plot(n_x1, n_y1, 'o', color='black')
        # plt.plot(n_x2, n_y2, 'o', color='black')
        # plt.show()
        ...
        for i in range(len(target['boxes'])):
            xc, yc, xw, yh = target['boxes'][i]
            _, h, w = img[i % len(img)].shape

            n_xc, n_yc, n_xw, n_yh = int(xc.item() * w), int(yc.item() * h), \
                                     int(xw.item() * w), int(yh.item() * h)

            x1, y1 = abs(n_xc - n_xw // 2), abs(n_yc - n_xw // 2)
            x2, y2 = abs(n_xc + n_xw // 2), abs(n_yc + n_xw // 2)
            # plt.imshow(torchvision.transforms.ToPILImage()(img[i]), interpolation="bicubic")
            # plt.plot(x1, y1, marker='s', color='black', markersize=12)
            # plt.plot(x2, y2, marker='8', color='black', markersize=12)
            # plt.show()

            ins_ann = img[i % len(img)][:, y1:y2, x1:x2]
            # plt.imshow(torchvision.transforms.ToPILImage()(ins_ann), interpolation="bicubic")
            # plt.show()
            ins_ann = ins_ann.permute(1,2,0).flatten().resize_(64).unsqueeze(0)
            ins_anns.append(ins_ann)

        ins_anns = torch.cat(ins_anns)
        target.update({"ins_ann": ins_anns})
...
def build(image_set, args):
    ...
    mode = 'instances'
    PATHS = {
        "train": (root / "vos/train/JPEGImages", root / "vis/2019/train.json"),
        "val": (root / "vos/valid/JPEGImages", root / "vis/2019/valid.json"),
    }
