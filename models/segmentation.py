...
class VisTRsegm(nn.Module):
    def __init__(self, vistr, freeze_vistr=False):
        ...
        self.gmm = GMNet()
        self.spp = SPPLayer(8)
        self.fc = nn.Linear(64, 42)
        self.proposal2matching = nn.Linear(384, 64)
        ...
        self.batch_size = vistr.batch_size
    ...
    def forward(self, samples: NestedTensor):
        ...
        ############################################################
        # [b, t * num_instance, fea]
        proposal_ins = hs.mean(0)
        proposal_ins = self.proposal2matching(proposal_ins)
        out = {"pred_ins": proposal_ins}
        ############################################################
        ...
        for i in range(self.vistr.num_frames):
            ...
            seg_masks_f = self.mask_head(memory_f, bbox_mask_f,
                                         [features[2].tensors[:,i],
                                          features[1].tensors[:,i],
                                          features[0].tensors[:,i]])
        ...
        outputs_seg_masks_ = self.spp(outputs_seg_masks)
        outputs_class = self.fc(outputs_seg_masks_)
        out["pred_logits2"] = outputs_class
